...
function SidebarLink({ to, label, icon: Icon, onNavigate }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `group relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-200 ${
          isActive ? "bg-ember/10 text-ember" : "text-ash hover:bg-panel hover:text-chalk"
        }`
      }
    >
      {({ isActive }) => (
  ...
  return (
    <>
      <div className="px-6 py-6 border-b border-line">
        ...
        <div className="text-xs text-ash mt-1.5 pl-9">Ministry of Coal · Carbon Tracker</div>
      </div>

      <nav className="flex-1 px-3 py-4 space-y-5 overflow-y-auto">
        <div>
          <div className="px-3 mb-2 text-[10px] uppercase tracking-wider text-ash">Main</div>
          <div className="space-y-1">
            {mainLinks.map((link) => (
        ...
        {user?.role === "moc_admin" && (
          <div>
            ...
            <SidebarLink to="/audit-logs" label="Audit Logs" icon={ShieldCheck} onNavigate={onNavigate} />
          </div>
        )}
      </nav>
      ...
      <div className="px-4 py-4 border-t border-line">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="min-w-0">
            <div className="text-sm text-chalk truncate">{user?.name || "User"}</div>
            <div className="text-xs text-ash truncate">
              {user?.role === "moc_admin" ? "Ministry Admin" : "Mine Admin"}
            </div>
          </div>
        ...
        <button onClick={logout} className="btn-secondary text-sm w-full flex items-center justify-center gap-2">
          <LogOut size={15} />
          Log out
        </button>
      </div>
    ...
    <>
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-40 flex">
          ...
          <aside className="relative z-50 w-72 max-w-[80vw] h-full bg-seam border-r border-line flex flex-col">
            <button
